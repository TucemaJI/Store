namespace Store.Shared.Constants
{
    public partial class Constants
    {
        public class EmailConsts
        {
            public const string CONFIRM_EMAIL = "ConfirmEmail";
            public const string ACCOUNT = "Account";
            public const string CONFIRM_ACOUNT = "Confirm your account";
            public const string COMPLITING_REGISTRATION = "To complete the registration, check your email and follow the link provided in the letter";
            public const string EMAIL_CONFIRMED = "Email Confirmed";
            public const string NEW_PASSWORD = "Your new password";

        }
    }
}
